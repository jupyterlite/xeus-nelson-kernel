...
def create_env(
    ...
    raise RuntimeError(
        """Failed to create the virtual environment for xeus-nelson,
...
def build_and_pack_emscripten_env(
    ...
    xeus_nelson_version: str = "",
    packages: List[str] = [],
    environment_file: str = "",
    root_prefix: str = "/tmp/xeus-nelson-kernel",
    env_name: str = "xeus-nelson-kernel",
    empack_config: str = "",
    output_path: str = ".",
    ...
    specs = [
        ...
        "xeus-lite",
        "xeus-nelson"
        if not xeus_nelson_version
        else f"xeus-nelson={xeus_nelson_version}",
        *packages,
    ]
    bail_early = True

    if packages or xeus_nelson_version or environment_file:
        bail_early = False
        ...
        if env_data.get("dependencies") is not None:
            ...
            for dependency in dependencies:
                if isinstance(dependency, str) and dependency not in specs:
                    ...
                    raise RuntimeError(
                        """Cannot install pip dependencies in the xeus-nelson Emscripten environment (yet?).
                        """
                    )
    ...
    try:
        ...
        # Copy xeus-nelson output
        for file in ["xnelson_wasm.js", "xnelson_wasm.wasm"]:
        ...
        if build_worker:
            ...
            worker = worker.replace("XEUS_KERNEL_FILE", "'xnelson_wasm.js'")
            worker = worker.replace("LANGUAGE_DATA_FILE", "''")
            #worker = worker.replace("LANGUAGE_DATA_FILE", "'nelson_data.js'")
...
def main(
    ...
    build_worker: bool = typer.Option(
        False,
        help="Whether or not to build the TypeScript worker code for using xeus-nelson in JupyterLite",
    ),
):
